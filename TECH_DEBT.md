...
`Data/Seeders/PaymentTestSeeder.cs` injects `Concertable.B2B.Seed.Infrastructure.SeedState`
(aliased `B2BSeedState`) and `Concertable.Customer.Seed.SeedState` (`CustomerSeedState`).

This crosses service boundaries — Payment compiles against B2B's internal Domain
(via `SeedState`) and Customer's seed data. In the microservice model Payment must
...
of importing the other services' `SeedState`. Then drop the two `ProjectReference`s.
